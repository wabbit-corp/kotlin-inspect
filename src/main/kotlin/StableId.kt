...
/**
 * Thread-safe helper that computes a *stable* textual identifier for functions/constructors **and**
 * classes.
 *
 ...
 *
 * which survives any clean rebuild as long as you don’t change the relative position of the lambda
 * inside its enclosing method.
 *
...
object StableId {
    ...
    fun of(kcls: KClass<*>): String = clsCache.computeIfAbsent(kcls.java) { computeForClass(it) }

    /** Stable id of a class (java.lang.Class overload). */
    fun of(jcls: Class<*>): String = clsCache.computeIfAbsent(jcls) { computeForClass(it) }
    ...
    private fun computeForLambda(lambda: Function<*>): String {
        throw UnsupportedOperationException("StableId.of(lambda) is unsupported.")
        ...
        //                lambdaCls.declaredMethods.joinToString {
        // "${it.name}${Type.getMethodDescriptor(it)}" })
        //        val samInterface = lambdaCls.interfaces.first()        //
        ...
        //            println("λ-debug  owner method  -> ${m.name} ::
    ...
    private fun indyOrdinalAndLines(
        ...
        mn.instructions?.forEach { insn ->
            when (insn) {
                is org.objectweb.asm.tree.LineNumberNode -> lines += insn.line

                is org.objectweb.asm.tree.InvokeDynamicInsnNode -> {
                    println(
                        "λ-debug  InvokeDynamicInsnNode: " +
    ...
    private fun buildIdFromSerializedLambda(sl: SerializedLambda): String {
        ...
        val mn =
            cn.methods.firstOrNull { it.name == implName && it.desc == implDesc }
    ...
    private fun computeForFunction(kf: KFunction<*>): String {
        ...
        runCatching { kf.javaMethod }
            .getOrNull()
            ?.let {
                return ordinaryMethodId(it)
            }

        // Fast path: constructor
        runCatching { kf.javaConstructor }
            .getOrNull()
            ?.let {
                return ctorId(it)
            }
    ...
    private fun KFunction<*>.safeJavaMethod(): Method? = runCatching { javaMethod }.getOrNull()
    ...
    private fun ordinaryMethodId(m: Method): String =
        "${m.declaringClass.name}#" + "${m.name}${Type.getMethodDescriptor(m)}"
     ...
     * 1. We take the lambda **instance** (kf is backed by that object), grab its synthetic class.
     * 2. We locate the *enclosing* method by using `enclosingMethod()` metadata; if absent we
     *    inspect every method for the matching `invokedynamic`.
     * 3. Inside the enclosing method we iterate over `invokeDynamic` instructions, counting an
     *    *ordinal* until we find the one whose descriptor matches the lambda’s invoked type.
    ...
    /**
     * Walks the instructions of [mn]; returns *ordinal* of the first `invokedynamic` matching
     * [targetDesc] and all line numbers seen in that method.
     *
    ...
    private fun loadAsm(jcls: Class<*>): ClassNode =
        jcls.getResourceAsStream("/${jcls.name.replace('.', '/')}.class")?.use { stream ->
