...
class StableIdTests {
    ...
    @Test fun `top-level function id contains name`() {
        val id = StableId.of(::sampleTopLevel)
        assertTrue(id.contains("#sampleTopLevel("))
    }

    @Test fun `member method id contains class and name`() {
        val id = StableId.of(SampleClass::intFun)
        assertTrue(id.contains("SampleClass#intFun("))
    }

    @Test fun `static companion method id contains class and name`() {
    ...
    @Test fun `different constructors yield different ids`() {
        val primary   = SampleClass::class.constructors
            .first { it.parameters.size == 1 }
        val secondary = SampleClass::class.constructors
    ...
    @Test fun `overloaded methods have distinct ids`() {
        val intFun = OverloadClass::class.functions
            .single { it.name == "process" && it.parameters.last().type.classifier == Int::class }
        val strFun = OverloadClass::class.functions
            .single { it.name == "process" && it.parameters.last().type.classifier == String::class }
    ...
    @Ignore
    @Test fun `FAILING different lambdas on different lines differ`() {
    ...
    @Ignore
    @Test fun `FAILING lambda id repeats identically for same instance`() {
    ...
    @Test fun `class id equals canonical name for ordinary class`() {
        val id = StableId.of(SampleClass::class)
        assertEquals("one.wabbit.inspect.SampleClass", id)
    }

    @Test fun `anonymous class id tagged with anon`() {
        val anonObj = object : Runnable { override fun run() {} }
    ...
    @Test fun `StableId reuses cache on subsequent calls`() {
